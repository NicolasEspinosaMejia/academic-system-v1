...
class StudentService(Service):
    ...
    def associate_student_course(self, data):
        """[summary]
        ...
        if not self.check_data_associate_student_course(data):
            raise ValueError(
                "The validation does not meet the minimum requirements")

        response = self.compose.associate_student_course(data)

        if response is not None:
            result["details"] = "Successfully associate student and course"

        return result

    def associate_student_address(self, data):
        """[summary]
        ...
        if not self.check_data_associate_student_address(data):
            raise ValueError(
                "The validation does not meet the minimum requirements")

        response = self.compose.associate_student_address(data)

        if response is not None:
            result["details"] = "Successfully associate student and address"

        return result

    def disassociate_student_course(self, data):
        ...
        response = self.compose.disassociate_student_course(data)
    ...
    def disassociate_student_address(self, data):
        """This endpoint is in charge of managing the students of the academy,
           through an action that they carry out[summary]
        ...
        response = self.compose.disassociate_student_address(data)
    ...
    def check_data_associate_student_course(self, data):
        ...
        if "identifierCourse" not in data or\
           not isinstance(data["identifierCourse"], int):
            result = False

        return result

    def check_data_associate_student_address(self, data):
        """This method is in charge of performing a check on the
           attributes that the methods that use it must have mandatory
        ...
        if "identifierAddress" not in data or\
           not isinstance(data["identifierAddress"], int):
            result = False
